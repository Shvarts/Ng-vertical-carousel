...
(function () {
    ...
    function Carousel3dController($scope, $element, $attrs, $timeout, $log, $parse, Carousel3dService) {
        ...
        function setCss(i, zIndex, positive) {
            var x = [160, 325, 465, 650],
                z = [280, 800, 1300, 2000],
            //(carousel3d.space == "auto") ? parseInt((i + 4) * (carousel3d.width / 1.5)) : parseInt((i + 3) * (carousel3d.space)),
                transform = (positive) ?
                            'translateX(' + (x[i]) + 'px) translateZ(-' + (z[i]) + 'px) rotateY(-' + carousel3d.perspective + 'deg)' :
                            'translateX(-' + (x[i]) + 'px) translateZ(-' + (z[i]) + 'px) rotateY(' + carousel3d.perspective + 'deg)',
                // left = "0%",
